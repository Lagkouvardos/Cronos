...
# Create the transition matrix for each combination of timepoints and save them

transitions <- matrix(0,nrow = max(samples_on_clusters,na.rm = T), ncol = max(samples_on_clusters,na.rm = T))


for (i in 1:ncol(infants)-1){
    for (k in 1: nrow(infants)){
      transitions[as.numeric(infants[k,i]), as.numeric(infants[k,i+1])] = transitions[infants[k,i], infants[k,i+1]] +1 
   }  
}

transitions <- t(apply(transitions,MARGIN = 1 ,FUN = function(x){x/sum(x)}))
######################### END OF SECTION #################################################
...
apply(samples_on_clusters, MARGIN = 2, FUN = max)
rownames(samples_on_clusters)
...
dudi.pca(df = otu_file,scannf = F,nf = 2)
