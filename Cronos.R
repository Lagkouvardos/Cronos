...
for (i in 1:(ncol(dataset_full_on_clusters)-1)){
  for (j in min(dataset_full_on_clusters[,i],na.rm = T):max(dataset_full_on_clusters[,i],na.rm = T)){
    for (k in min(dataset_full_on_clusters[,i+1],na.rm = T):max(dataset_full_on_clusters[,i+1],na.rm = T)){
      ...
      matrix_of_transitions[j,k] = sum(dataset_full_on_clusters[dataset_full_on_clusters[,i]==j,(i+1)]==k,na.rm = T)/sum(dataset_full_on_clusters[,i]==j & !is.na(dataset_full_on_clusters[,i+1]),na.rm = T)
...
for (end_timepoint in head(as.numeric(rev(colnames(dataset_full))),n = (ncol(dataset_full))-1)){
  ...
  legend("topright", c("Test_sets_LOO","Test_sets_stratified", 'Train_sets_LOO','Train_sets_stratified'), fill=c('paleturquoise4','paleturquoise3', 'darkolivegreen2', 'darkolivegreen3'), cex = 1)
...
write.csv(x = rbind(paste(paste('Maximum Accuracy for Timepoint', rev(colnames(dataset_full)[2:ncol(dataset_full)])), c(rev(colnames(dataset_full))[2:ncol(dataset_full)]), sep = ' from Timepoint '),maxaccuracies,metadata,random_estimator[1:length(random_estimator)-1]),file = paste(output_dir,'Maximum_Accuracies_of_Train_sets_LOO.csv',sep = '/'),row.names = F)
...
write.csv(x = rbind(paste(paste('Maximum Accuracy for Timepoint', rev(colnames(dataset_full)[2:ncol(dataset_full)])), c(rev(colnames(dataset_full))[2:ncol(dataset_full)]), sep = ' from Timepoint '),maxaccuracies,metadata,random_estimator[1:length(random_estimator)-1]),file = paste(output_dir,'Maximum_Accuracies_of_TrainSplits.csv',sep = '/'),row.names = F)
